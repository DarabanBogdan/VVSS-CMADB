...
public class ArrayTaskList extends TaskList{
    ...
    public void add(Task task){
        ...
        if (numberOfTasks == currentCapacity-1){
            final int new_capacity=2;
            currentCapacity = currentCapacity * new_capacity;
    ...
    protected ArrayTaskList clone() throws CloneNotSupportedException {
        ArrayTaskList tasks_list = new ArrayTaskList();
        for (int i = 0; i < this.tasks.length; i++){
            tasks_list.add(this.getTask(i));
        }
        return tasks_list;
